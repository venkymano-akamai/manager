...
import {
  ...
  errorUploadRulesFactory,
  flagsFactory,
  notificationChannelFactory,
  serviceAlertFactory,
  serviceTypesFactory,
  streamFactory,
  successUploadRulesFactory,
...
const customAlertDefinition = alertDefinitionFactory.build({
  ...
  label: 'Alert-1',
  rule_criteria: {
    rules: [successUploadRulesFactory.build(), errorUploadRulesFactory.build()],
  },
  severity: 0,
  tags: [''],
...
describe('Creates a log service alert with valid configuration', () => {
  /*
  ...
  it('should successfully create a new alert for Enity level', () => {
    ...
    mockGetCloudPulseServices(['logs']);
    mockGetCloudPulseMetricDefinitions('logs', metricDefinitions);
    ...
    const alerts = alertFactory.build({
      ...
      rule_criteria: {
        rules: [
          successUploadRulesFactory.build(),
          errorUploadRulesFactory.build(),
        ],
      },
      service_type: 'logs',
      severity: 0,
    ...
    mockCreateAlertDefinition(serviceType, alerts).as('createAlertDefinition');
    ...
    ui.autocomplete.findByLabel('Service').type('Logs');
    ui.autocompletePopper.findByTitle('Logs').click();
    ...
    fillMetricDetailsForSpecificRule(successMetricDetails);
    ...
    cy.findByText('StatusCode').should('be.visible').click();
    ...
    cy.findByPlaceholderText('e.g., 200').type('200');

    // Fill metric details for the second rule

    const errorMetricDetails = {
      aggregationType: 'Avg',
      dataField: 'Error Upload Count',
      operator: '=',
      ruleIndex: 1,
      threshold: '1000',
    };

    fillMetricDetailsForSpecificRule(errorMetricDetails);
    ...
    cy.wait('@createAlertDefinition').then(({ request, response }) => {
      ...
      expect(response?.body.scope).to.eq('entity');
